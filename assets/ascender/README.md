...
| `ascender.usd` | sim-ready **cam head only** (rope channel + locking mechanism; handle loop and grip cut away at model z = 0.60). Scaled so the full tool would be **195 mm** (head ≈ 76 mm), **110 g**, textures re-bound from `textures/`, convex-hull collision (invisible), `RigidBodyAPI` + `MassAPI`. Frame = full-tool frame (origin at the old handle bottom, +Z up through the head). |

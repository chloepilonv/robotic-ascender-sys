# app/monitoring/sim — test the monitoring stack in MuJoCo (no robot needed)

Same output schema as `app/monitoring/real/log.jsonl`, plus sim-only keys (`base_pos_m`, `speed_mps`, `cost_of_transport`,
...
## 2. Run the demo (robot stands, swings arms; 30 s sim at 5,300 m in −19 °C, 30 km/h wind)
    python app/monitoring/sim/mujoco_monitor.py --xml mujoco_menagerie/unitree_g1/g1.xml --seconds 30 --altitude 5300 --wind 30
    python app/monitoring/sim/mujoco_monitor.py --xml ... --soc0 30          # start with a low pack
    python app/monitoring/sim/mujoco_monitor.py --xml ... --viewer           # open the MuJoCo window
Writes `app/monitoring/sim/log.jsonl`, one line per sim second → point the app at that file.

## 3. Hook into your own policy loop
    from app.monitoring.sim.mujoco_monitor import SimMonitor, Environment
    mon = SimMonitor(model, Environment(altitude_m=5300, wind_kmh=30), soc0=100, log="app/monitoring/sim/log.jsonl")
    while ...:

...
Library use (inside your own policy loop):
    mon = SimMonitor(model, env=Environment(altitude_m=5300, wind_kmh=30), log="app/monitoring/bms/sim/log.jsonl")
    while running:
...
Standalone demo (no policy, PD hold of the standing keyframe + arm swing), 60 s of sim in a few s real time:
    python app/monitoring/bms/sim/mujoco_monitor.py --xml path/to/mujoco_menagerie/unitree_g1/g1.xml --seconds 60 --altitude 5300
"""
